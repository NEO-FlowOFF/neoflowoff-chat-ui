...
export async function upsertLead(lead: Lead): Promise<void> {
  ...
  let wasAlreadyQualified = false;
  let existingEmail = "";
  let existingTelefone = "";

  try {
    const existing = await pool.query(
      `SELECT email, telefone, qualificado FROM leads WHERE session_id = $1`,
      [lead.sessionId]
    );
    if (existing.rows.length > 0) {
      wasAlreadyQualified = !!existing.rows[0].qualificado;
  ...
  // 2. Determina se o lead agora é qualificado (precisa ter pelo menos um canal de contato)
  const finalEmail = (lead.email || existingEmail || "").trim();
  const finalTelefone = (lead.telefone || existingTelefone || "").trim();

  const isQualified = !!(finalEmail || finalTelefone);

  // 3. Insere ou atualiza no banco com a classificação correta
  await pool.query(
    `
    INSERT INTO leads (session_id, nome, email, telefone, empresa, observacoes, qualificado)
  ...
  // 4. Dispara e-mail via Resend somente na transição de não qualificado -> qualificado
  if (isQualified && !wasAlreadyQualified) {
    ...
    sendResendNotification(mergedLead).catch((err) => {
